...
try:
	...
	if len(all_results) > 0:
		...
		if adminstatus:
			...
			print "'<h3>' + dat.viewer + '</h3><p>' + dat.comment + '</p><p>Rating: ' + dat.score + '</p>'"
			print "},})});});"
			print "</script>"
			print "<script>"
			print "$(document).ready(function(){"
			print "$('usr_name').defaultValue = usr;"
			#print "$('usr_name').style.visibility='hidden';"
			print "});"
			print "</script>"
			print "</head>"
			print "<body>"
			...
			print "<label>Enter the applicant you want to comment on:</label><input name = 'usr_name' type='text' value = usr id='usr_name'/>"
